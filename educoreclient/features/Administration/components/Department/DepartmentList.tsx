...
interface DepartmentListProps {
  ...
  sortOrder?: 'asc' | 'desc';
  onSortChange?: (column: keyof Department) => void;
...
const DepartmentList: React.FC<DepartmentListProps> = ({ departments, loading, onEdit, onDelete, sortBy, sortOrder, onSortChange, bulkUpdateMode, isSelected, isAllSelected, isIndeterminate, toggleItem, toggleSelectAll }) => {
      ...
      <table className="min-w-full divide-y divide-slate-200/60">
        <thead className="bg-slate-50/50">
          <tr>
            ...
            <th className="px-4 py-2.5 text-left text-xs font-medium text-slate-600 uppercase tracking-wider">
              <button type="button" className="inline-flex items-center space-x-1" onClick={() => onSortChange?.('departmentName')}>
            ...
            <th className="px-4 py-2.5 text-left text-xs font-medium text-slate-600 uppercase tracking-wider">
              <button type="button" className="inline-flex items-center space-x-1" onClick={() => onSortChange?.('departmentCode')}>
        ...
        <tbody className="bg-white divide-y divide-slate-200/60">
          {loading ? (
            ...
            departments.map((department: Department) => (
              <tr key={department.id} className="hover:bg-slate-50/50 transition-colors duration-150">
                {bulkUpdateMode && (
                  <td className="px-4 py-3 whitespace-nowrap">
                    <input
                      type="checkbox"
                      checked={isSelected?.(department.id)}
                      onChange={() => toggleItem?.(department.id)}
                      className="form-checkbox h-4 w-4 text-blue-600 border-gray-300 rounded"
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="text-xs font-medium text-slate-900">{department.departmentName}</div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="text-xs font-medium text-slate-900">{department.departmentCode}</div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeClasses(department.isActive)}`}>
