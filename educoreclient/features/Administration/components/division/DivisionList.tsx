...
interface DivisionListProps {
  ...
  sortOrder?: 'asc' | 'desc';
  onSortChange?: (column: keyof Division) => void;
...
const DivisionList: React.FC<DivisionListProps> = ({ divisions, loading, onEdit, onDelete, sortBy, sortOrder, onSortChange, bulkUpdateMode, isSelected, isAllSelected, isIndeterminate, toggleItem, toggleSelectAll }) => {
      ...
      <table className="min-w-full divide-y divide-slate-200/60">
        <thead className="bg-slate-50/50">
          <tr>
            ...
            <th className="px-4 py-2.5 text-left text-xs font-medium text-slate-600 uppercase tracking-wider">
              <button type="button" className="inline-flex items-center space-x-1" onClick={() => onSortChange?.('divisionName')}>
            ...
            <th className="px-4 py-2.5 text-left text-xs font-medium text-slate-600 uppercase tracking-wider">
              <button type="button" className="inline-flex items-center space-x-1" onClick={() => onSortChange?.('divisionCode')}>
        ...
        <tbody className="bg-white divide-y divide-slate-200/60">
          {loading ? (
            ...
            divisions.map((division: Division) => (
              <tr key={division.id} className="hover:bg-slate-50/50 transition-colors duration-150">
                {bulkUpdateMode && (
                  <td className="px-4 py-3 whitespace-nowrap">
                    <input
                      type="checkbox"
                      checked={isSelected?.(division.id)}
                      onChange={() => toggleItem?.(division.id)}
                      className="form-checkbox h-4 w-4 text-blue-600 border-gray-300 rounded"
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="text-xs font-medium text-slate-900">{division.divisionName}</div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="text-xs font-medium text-slate-900">{division.divisionCode}</div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeClasses(division.isActive)}`}>
                    {division.isActive ? 'Yes' : 'No'}
                  </span>
                </td>
