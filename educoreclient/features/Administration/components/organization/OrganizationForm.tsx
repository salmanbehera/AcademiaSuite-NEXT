...
export default function OrganizationForm({
  ...
  onSubmit: (data: Organization) => Promise<{ id: string } | undefined>;
  ...
  const [country, setCountry] = useState(() => {
    if (defaultValues && defaultValues.country) {
      ...
      const found = countriesData.find(
        (c: any) =>
          c.name === defaultValues.country || c.id === defaultValues.country
      );
  ...
  const [state, setState] = useState(() => {
    if (defaultValues && defaultValues.state) {
      ...
      const found = statesData.find(
        (s: any) =>
          s.name === defaultValues.state || s.id === defaultValues.state
      );
  ...
  const [city, setCity] = useState(() => {
    if (defaultValues && defaultValues.city) {
      ...
      const found = citiesData.find(
        (c: any) => c.name === defaultValues.city || c.id === defaultValues.city
      );
      return found ? found.id : "";
    }
    return "";
  ...
  const stateOptions = statesData
    .filter((s: any) => s.countryId === country)
    .map((s: any) => ({ value: s.id, label: s.name }));
  const cityOptions = useMemo(() => {
    return citiesData
      .filter((ct: any) => ct.stateId === state)
  ...
  useEffect(() => {
    setCity("");
    const stateLabel =
      statesData.find((s: any) => s.id === state && s.countryId === country)
        ?.name || "";
    setFormData((prev) => ({
      ...prev,
      state: stateLabel,
    ...
    const cityLabel =
      citiesData.find(
        (ct: any) =>
          ct.id === city &&
          statesData.find(
            (s: any) => s.id === ct.stateId && s.countryId === country
          )
      )?.name || "";
    setFormData((prev) => ({
      ...prev,
    ...
    if (defaultValues) {
      ...
      if (defaultValues.city) {
        const foundCity = citiesData.find(
          (c: any) =>
            c.name === defaultValues.city || c.id === defaultValues.city
        );
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Map server validation errors into formErrors using shared utility
      const { extractValidationErrors, applyValidationToForm } = await import(
        "@/lib/validationUtils"
      );
      const handled = applyValidationToForm(
        (updater: any) => setFormErrors(updater),
        error as any
      );
      if (!handled) {
        // No validation payload — rethrow so parent can show a toast
        throw error;
      }
    } finally {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (
        ...
        showError(
          "Update Failed",
          "Failed to update the organization. Please try again."
        );
      }
    } catch (error) {
      const { applyValidationToForm } = await import("@/lib/validationUtils");
      const handled = applyValidationToForm(
        (updater: any) => setFormErrors(updater),
        error as any
      );
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Input
            ...
            onChange={handleChange}
            error={formErrors.affiliationBoard}
        ...
        <div>
          ...
          <Select
            ...
            onChange={(value) =>
              handleSelectChange("organizationType", Number(value))
            }
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Input
            id="panTinTaxId"
            name="panTinTaxId"
            value={formData.panTinTaxId ?? ""}
            onChange={handleChange}
            error={formErrors.panTinTaxId}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Input
            ...
            onChange={handleChange}
            error={formErrors.headOfOrganization}
          ...
          <Input
            ...
            value={
              formData.academicYearStart
                ? formData.academicYearStart.substring(0, 10)
                : ""
            }
            onChange={handleChange}
          ...
          <Input
            ...
            value={
              formData.academicYearEnd
                ? formData.academicYearEnd.substring(0, 10)
                : ""
            }
            onChange={handleChange}
