...
export class DepartmentService {
  ...
  static async getDepartments(
  ...
  ): Promise<GetDepartmentsResponse["result"]> {
    ...
    const response = await enterpriseApiClient.post<GetDepartmentsResponse>(
  ...
  static async getDepartmentById(id: string): Promise<Department> {
    const response = await enterpriseApiClient.get<{ result: Department }>(
      `${this.BASE}/${id}`
    );
    return response.result;
  }
  ...
  static async createDepartment(
    ...
    if (process.env.NODE_ENV === "development") {
      // Log the exact request payload we send to the server for debugging
      // Helps diagnose malformed JSON / unexpected wrapper issues
      // eslint-disable-next-line no-console
      console.debug("[DepartmentService] POST payload ->", request);
    }
    const response = await enterpriseApiClient.post<{ result: Department }>(
      this.BASE,
      request
    );
    return response.result;
  ...
  static async updateDepartment(
    ...
    const response = await enterpriseApiClient.put<{ result: Department }>(
      this.BASE,
      payload
    );
    return response.result;
  ...
  static async bulkCreateDepartments(
    ...
    const payload = {
      departments: departmentsData.map((data) => this.wrapDepartment(data)),
    };
    const response = await enterpriseApiClient.post<{ result: Department[] }>(
      `${this.BASE}/bulk`,
      payload
  ...
  static async importDepartments(importData: DepartmentDto[]): Promise<void> {
    await enterpriseApiClient.post<void>(`${this.BASE}/import`, {
      departments: importData,
    });
  }

  static async exportDepartments(format: "csv" | "excel"): Promise<Blob> {
    const response = await enterpriseApiClient.get<Blob>(
      `${this.BASE}/export?format=${format}`
    );
    return response;
  }

  static async bulkDeleteDepartments(departmentIds: string[]): Promise<void> {
    await enterpriseApiClient.post<void>(`${this.BASE}/bulk-delete`, {
      departmentIds,
    });
  }
}
