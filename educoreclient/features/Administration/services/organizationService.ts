...
import {
  Organization,
  OrganizationDto,
} from "@/features/Administration/types/organizationTypes";
...
export class OrganizationService {
  ...
  static async getOrganizations(
    ...
    const normalizedParams = {
      pageIndex:
        params?.pageIndex ?? API_CONFIG.PAGINATION?.DEFAULT_PAGE_INDEX ?? 0,
      pageSize:
        params?.pageSize ?? API_CONFIG.PAGINATION?.DEFAULT_PAGE_SIZE ?? 10,
      ...params,
    };
    ...
    organizations = organizations.map((org: Organization) => ({
      ...
      academicYearStart: org.academicYearStart
        ? org.academicYearStart.split("T")[0]
        : org.academicYearStart,
      academicYearEnd: org.academicYearEnd
        ? org.academicYearEnd.split("T")[0]
        : org.academicYearEnd,
    }));
    let count =
      typeof result?.count === "number" ? result.count : organizations.length;
    let pageIndexResp =
      typeof result?.pageIndex === "number"
        ? result.pageIndex
        : normalizedParams.pageIndex;
    let pageSizeResp =
  ...
  static async getOrganizationById(id: string): Promise<Organization> {
    ...
    const response = await apiClient.get(`${this.BASE}/${cleanedId}`);
  ...
  static async updateOrganization(id: string, data: Partial<OrganizationDto>) {
    ...
    const filteredData: Partial<Record<string, any>> = Object.entries({
      id,
      ...data,
    })
      .filter(
  ...
  static async bulkCreateOrganizations(
  ...
  static async exportOrganizations(
