...
import { FeeGroup } from '@/features/student/types/FeeManagement/feegroupType';
...
interface FeeGroupListProps {
  ...
  sortOrder?: 'asc' | 'desc';
  onSortChange?: (column: keyof FeeGroup) => void;
...
const FeeGroupList: React.FC<FeeGroupListProps> = ({ feegroups, loading, onEdit, onDelete, sortBy, sortOrder, onSortChange, bulkUpdateMode, isSelected, isAllSelected, isIndeterminate, toggleItem, toggleSelectAll }) => {
      ...
      <table className="min-w-full divide-y divide-slate-200/60">
        <thead className="bg-slate-50/50">
          <tr>
            ...
            <th className="px-4 py-2.5 text-left text-xs font-medium text-slate-600 uppercase tracking-wider">
              <button type="button" className="inline-flex items-center space-x-1" onClick={() => onSortChange?.('FeeGroupCode')}>
                <span>Fee Group Code</span>
                {sortBy === 'FeeGroupCode' ? (
            ...
            <th className="px-4 py-2.5 text-left text-xs font-medium text-slate-600 uppercase tracking-wider">
              <button type="button" className="inline-flex items-center space-x-1" onClick={() => onSortChange?.('FeeGroupName')}>
                <span>Fee Group Name</span>
                {sortBy === 'FeeGroupName' ? (
                  sortOrder === 'asc' ? <ChevronUp size={12} /> : <ChevronDown size={12} />
                ) : (
                  <ArrowUpDown size={12} className="opacity-50" />
            ...
            <th className="px-4 py-2.5 text-left text-xs font-medium text-slate-600">Description</th>
        ...
        <tbody className="bg-white divide-y divide-slate-200/60">
          {loading ? (
            ...
            feegroups.map((fg: FeeGroup) => (
              <tr key={fg.id} className="hover:bg-slate-50/50 transition-colors duration-150">
                {bulkUpdateMode && (
                  <td className="px-4 py-3 whitespace-nowrap">
                    <input
                      type="checkbox"
                      checked={isSelected?.(fg.id)}
                      onChange={() => toggleItem?.(fg.id)}
                      className="form-checkbox h-4 w-4 text-blue-600 border-gray-300 rounded"
                      title="Select Fee Group"
                    />
                  </td>
                )}
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="flex items-center">
                    <div className="w-8 h-8 bg-gradient-to-br from-slate-700 to-slate-900 rounded-lg flex items-center justify-center shadow-sm">
                      <span className="text-white font-semibold text-xs">{fg.FeeGroupCode}</span>
                    </div>
                    <div className="ml-3">
                      <div className="text-xs font-medium text-slate-900">{fg.FeeGroupCode}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="text-xs font-medium text-slate-900">{fg.FeeGroupName}</div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="text-xs font-medium text-slate-900">{fg.Description}</div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getActiveBadgeClasses(fg.IsActive)}`}>
                    {fg.IsActive ? 'Yes' : 'No'}
                  </span>
                </td>
