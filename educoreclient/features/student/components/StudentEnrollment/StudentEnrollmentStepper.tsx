...
export default function StudentEnrollmentStepper({
  initialData,
  onSubmit,
}: {
  initialData?: any;
  ...
  const handleChange = (section: string, value: any) => {
    ...
    try {
      methods.setValue(section, value);
    } catch (e) {
      // ignore if setValue not applicable
    }
  };
      ...
      <div className="min-h-screen w-full bg-gradient-to-br from-blue-50 to-white rounded shadow pl-4 pr-4 overflow-x-auto">
        ...
        <div className="flex gap-8 mb-8">
            ...
            {steps.map((label, idx) => (
              <React.Fragment key={label}>
                ...
                >
                  ...
                  <span
                    className={`ml-3 text-[12px] font-medium whitespace-nowrap ${
          ...
          <div className="flex-1">
            ...
            {step === 1 &&
              (loadingYears || loadingClasses || loadingSections ? (
                <div className="py-8 text-center text-slate-500">
                  Loading class enrollment controls...
                </div>
              ) : academicYears?.length === 0 ||
                classes?.length === 0 ||
                sections?.length === 0 ? (
                <div className="py-8 text-center text-slate-500">
                  No academic years, classes, or sections available. Please set
                  up master data first.
                </div>
              ) : (
                <ClassEnrollmentForm
            ...
            {step === 11 && <HostelDetailsForm />}
            {step === 12 && <TransportDetailsForm />}
            {/* Navigation */}
