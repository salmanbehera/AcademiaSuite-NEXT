  ...
  if (defaultValues && defaultValues.id) {
    return {
      ...
      startDate: defaultValues.startDate
        ? defaultValues.startDate.substring(0, 10)
        : "",
      endDate: defaultValues.endDate
        ? defaultValues.endDate.substring(0, 10)
        : "",
    };
  } else {
...
export const SemisterForm: React.FC<SemisterFormProps> = ({
  onSubmit,
  defaultValues,
  onCancel,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If creating a new semester, don't send an empty id (prevents server GUID parse errors)
      if (!defaultValues || !defaultValues.id || apiData.id === "") {
      ...
      showSuccess(
        defaultValues && defaultValues.id
          ...
          : "Semister Created",
        defaultValues && defaultValues.id
          ? "Semister updated successfully."
          : "Semister created successfully."
      );
      resetForm();
    } catch (err) {
        ...
        <div className="flex items-center">
          <Checkbox
            ...
            onCheckedChange={(checked) =>
              setFormData({ ...formData, isCurrentSemester: Boolean(checked) })
            }
          >
            <Label
              htmlFor="isCurrentSemester"
              className="text-xs font-medium text-slate-700"
            >
              Is Current Semester?
            </Label>
