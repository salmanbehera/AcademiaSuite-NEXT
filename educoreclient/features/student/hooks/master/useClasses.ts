...
export function useClasses(options: UseClassesOptions = {}): UseClassesReturn {
  ...
  const updateClass = useCallback(
    async (id: string, classData: Partial<ClassDto>): Promise<Class | null> => {
      if (!isReady || !organizationId || !branchId) {
        setError("Organization context not ready");
        return null;
      }
  ...
  const importClasses = useCallback(
    async (
      ...
      try {
        ...
        const result = await new Promise<{ success: number; errors: any[] }>(
          (resolve, reject) => {
                  ...
                  try {
                    ...
                    headers.forEach((header, index) => {
                      ...
                      if (mappedField) {
                        ...
                        // Convert data types
                        if (
                          mappedField === "displayOrder" ||
                          mappedField === "maxStrength" ||
                          mappedField === "reservationSeats"
                        ) {
