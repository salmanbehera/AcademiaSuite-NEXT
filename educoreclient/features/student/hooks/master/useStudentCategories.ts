...
export function useStudentCategories(
  options: UseStudentCategoriesOptions = {}
  ...
  const createStudentCategory = useCallback(
    async (
      categoryData: Omit<
        StudentCategoryDto,
        "organizationId" | "branchId" | "id" | "createdAt" | "updatedAt"
      ...
      try {
        const fullCategoryData = withOrgData(
          categoryData
        ) as StudentCategoryDto;
        const newCategory = await StudentCategoryService.createStudentCategory(
          fullCategoryData
        );
  ...
  const updateStudentCategory = useCallback(
    async (
      id: string,
      categoryData: Partial<StudentCategoryDto>
    ): Promise<StudentCategory | null> => {
      ...
      try {
        const fullUpdateData = withOrgData({ ...categoryData, id });
  ...
  const deleteStudentCategory = useCallback(
    ...
    [handleApiError, organizationId, branchId]
  );

  // Toggle student category status
  const toggleStudentCategoryStatus = useCallback(
