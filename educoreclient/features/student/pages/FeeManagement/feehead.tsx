...
export default function FeeHeadPage() {
	...
	const handleSave = async (data: any) => {
		try {
			if (editing && editingRecord) {
				console.log('DEBUG: handleSave update', { editingRecord, editing, data });
			...
			showError('Error', `Failed to save fee head. ${(err instanceof Error ? err.message : '')}`);
		}
	};
	// Edit
	...
	const handleDelete = (id: string) => {
		setConfirmDialog({
			...
			message: "Are you sure you want to delete this fee head? This action cannot be undone.",
	...
	const handleBulkDelete = async () => {
		try {
			...
			clearSelection();
			showSuccess('Bulk Delete', `${selectedCount} fee head${selectedCount > 1 ? 's' : ''} deleted successfully.`);
		} catch (error) {
			setShowBulkDeleteDialog(false);
			showError('Bulk Delete Failed', 'Failed to delete selected fee heads. Please try again.');
		}
	};
	// Search
	...
	const handleImport = async (file: File) => {
		...
		try {
			...
			await import('@/features/student/services/FeeManagement/feeheadService').then(m => m.FeeHeadService.importFeeHeads(parsedData));
	...
	const handleExport = async (format: 'csv' | 'excel') => {
		const blob = await import('@/features/student/services/FeeManagement/feeheadService').then(m => m.FeeHeadService.exportFeeHeads(format));
			...
			<div className="max-w-7xl mx-auto p-4 space-y-4">
				...
				<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white rounded-lg shadow-sm border border-slate-200/60 p-2.5">
					<div>
						...
						<p className="mt-0.5 text-xs text-slate-600">Configure and manage fee heads for your institution</p>
					</div>
				...
				<div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
					<StatsCard title="Total" value={totalCount} subtitle="Fee Heads" icon={Plus} iconColor="text-slate-700" iconBgColor="bg-slate-100" valueColor="text-slate-900" />
					<StatsCard title="Active" value={feeheads.filter((s: any) => s.IsActive).length} subtitle="Active" icon={RefreshCw} iconColor="text-emerald-600" iconBgColor="bg-emerald-50" valueColor="text-emerald-700" />
				</div>

				{/* Search + More Actions */}
				<div className="bg-white rounded-lg shadow-sm border border-slate-200/60 p-2.5 flex items-center gap-4">
					<div className="flex-1">
						<SearchBox value={searchTerm} placeholder="Search by fee head code..." onChange={handleSearchChange} className="w-full" />
					</div>
					<MoreActionsButton
						items={[{
					...
					/>
								...
								<div className="bg-white rounded-lg shadow-sm border border-slate-200/60 overflow-x-auto">
									...
									<ConfirmationDialog
										...
										message={`Are you sure you want to delete ${selectedCount} selected fee head${selectedCount > 1 ? 's' : ''}? This action cannot be undone.`}
										type="danger"
										confirmText="Delete"
										cancelText="Cancel"
									/>
