...
import { useLatefeepolicy } from '@/features/student/hooks/FeeManagement/UseLatefeepolicy';
import LateFeepolicyForm from '@/features/student/components/FeeManagement/LateFeePolicy/LateFeepolicyForm';
import LateFeePolicyList from '@/features/student/components/FeeManagement/LateFeePolicy/LateFeePolicyList';
...
export default function LateFeePolicyPage() {
  ...
  const {
    ...
    toggleSelectAll,
    clearSelection,
  } = useBulkSelection(filteredLatefeepolicies);

  const handleSave = async (data: any) => {
    try {
      if (editing && editingRecord) {
        await updateLatefeepolicy(editingRecord.id, data);
      } else {
        await createLatefeepolicy(data);
      }
      setShowForm(false);
      setEditing(null);
      setEditingRecord(undefined);
    } catch (err) {
      handleApiError(err, editing ? 'update late fee policy' : 'create late fee policy');
      showError('Error', `Failed to save late fee policy. ${(err instanceof Error ? err.message : '')}`);
    }
  };
  ...
  const handleDelete = (id: string) => {
    setConfirmDialog({
      isOpen: true,
      title: 'Delete Late Fee Policy',
      message: 'Are you sure you want to delete this late fee policy? This action cannot be undone.',
  ...
  const confirmDelete = async (id: string) => {
    ...
    try {
      ...
      showSuccess('Delete', 'Late fee policy deleted successfully.');
    } catch (err) {
      setConfirmDialog((prev) => ({ ...prev, loading: false }));
      showError('Error', 'Failed to delete late fee policy.');
    }
  };
  ...
  const handleBulkDelete = async () => {
    try {
      await bulkDeleteLatefeepolicies(Array.from(selectedItems));
      setShowBulkDeleteDialog(false);
      clearSelection();
      showSuccess('Bulk Delete', `${selectedCount} late fee polic${selectedCount > 1 ? 'ies' : 'y'} deleted successfully.`);
    } catch (error) {
      setShowBulkDeleteDialog(false);
      showError('Bulk Delete Failed', 'Failed to delete selected late fee policies. Please try again.');
    }
  };
  const handleSearchChange = (term: string) => {
    setSearchTerm(term);
    if (term.length >= MIN_SEARCH_LENGTH || term === '') {
      searchLatefeepolicies(term);
    }
  };
      ...
      <div className="max-w-7xl mx-auto p-4 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white rounded-lg shadow-sm border border-slate-200/60 p-2.5">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-600">Configure and manage late fee policies for your institution</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <StatsCard title="Active" value={latefeepolicies.filter((s: any) => s.isActive).length} subtitle="Active" icon={RefreshCw} iconColor="text-emerald-600" iconBgColor="bg-emerald-50" valueColor="text-emerald-700" />
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200/60 overflow-x-auto">
          ...
          />
          <LateFeePolicyList
            latefeepolicies={filteredLatefeepolicies}
            loading={loading}
            onEdit={handleEdit}
        ...
        <Modal
          ...
          title={editing ? "Edit Late Fee Policy" : "Create New Late Fee Policy"}
          size="md"
        >
          <LateFeepolicyForm
            onSubmit={handleSave}
        ...
        <ImportDialog
          ...
          description="Upload a CSV or Excel file to import late fee policies in bulk. Download the sample template for correct format."
          sampleData={[{
            policyName: 'Monthly Late Fee',
            gracePeriodDays: 7,
            penaltyType: 'Fixed per Day',
            penaltyValue: 2.5,
            maxPenalty: 600,
            isActive: true,
          }]}
