...
export class FeeGroupService {
  ...
  static async getFeeGroups(
    params?: GetFeeGroupsRequest
  ): Promise<GetFeeGroupsResponse["result"]> {
    ...
    const response = await enterpriseApiClient.post<GetFeeGroupsResponse>(
  ...
  static async getFeeGroupById(id: string): Promise<FeeGroup> {
    const response = await enterpriseApiClient.get<{ result: FeeGroup }>(
      `${this.BASE}/${id}`
    );
    return response.result;
  }

  static async createFeeGroup(
    request: CreateFeeGroupRequest
  ): Promise<FeeGroup> {
    const response = await enterpriseApiClient.post<{ result: FeeGroup }>(
      this.BASE,
      request
    );
    return response.result;
  ...
  static async updateFeeGroup(
    id: string,
    data: Partial<FeeGroupDto>
  ): Promise<FeeGroup> {
    const pascalCaseData: Record<string, unknown> = { ...data };
    ...
    const response = await enterpriseApiClient.put<{ result: FeeGroup }>(
      this.BASE,
      payload
    );
    return response.result;
  ...
  static async toggleFeeGroupStatus(
    id: string,
    completeData: FeeGroupDto,
    isActive: boolean
  ): Promise<FeeGroup> {
    const payload = this.wrapFeeGroup({
      ...completeData,
      id,
      IsActive: isActive,
    });
  ...
  static async bulkCreateFeeGroups(
    feeGroupsData: FeeGroupDto[]
  ): Promise<FeeGroup[]> {
    const payload = {
      feeGroups: feeGroupsData.map((data) => this.wrapFeeGroup(data)),
    };
    const response = await enterpriseApiClient.post<{ result: FeeGroup[] }>(
      `${this.BASE}/bulk`,
      payload
  ...
  static async searchFeeGroups(
    params: GetFeeGroupsRequest & { search?: string }
  ): Promise<GetFeeGroupsResponse["result"]> {
    const normalizedParams = {
      pageIndex: params?.pageIndex ?? API_CONFIG.PAGINATION.DEFAULT_PAGE_INDEX,
      pageSize: params?.pageSize ?? API_CONFIG.PAGINATION.DEFAULT_PAGE_SIZE,
