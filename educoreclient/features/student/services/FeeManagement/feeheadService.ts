...
export class FeeHeadService {
  ...
  static async getFeeHeads(
    params?: GetFeeHeadsRequest
  ): Promise<GetFeeHeadsResponse["result"]> {
    ...
    const response = await enterpriseApiClient.post<GetFeeHeadsResponse>(
  ...
  static async getFeeHeadById(id: string): Promise<FeeHead> {
    const response = await enterpriseApiClient.get<{ result: FeeHead }>(
      `${this.BASE}/${id}`
    );
  ...
  static async createFeeHead(request: CreateFeeHeadRequest): Promise<FeeHead> {
    const response = await enterpriseApiClient.post<{ result: FeeHead }>(
      this.BASE,
      request
    );
    return response.result;
  ...
  static async updateFeeHead(
    ...
    const response = await enterpriseApiClient.put<{ result: FeeHead }>(
      this.BASE,
      payload
    );
    return response.result;
  ...
  static async toggleFeeHeadStatus(
    id: string,
    completeData: FeeHeadDto,
    isActive: boolean
  ): Promise<FeeHead> {
    // Always ensure id is present in the payload and use wrapFeeHead
    const payload = this.wrapFeeHead({
      ...completeData,
      id,
      IsActive: isActive,
    });
  ...
  static async bulkCreateFeeHeads(
    feeHeadsData: FeeHeadDto[]
  ): Promise<FeeHead[]> {
    const payload = {
      feeHeads: feeHeadsData.map((data) => this.wrapFeeHead(data)),
    };
    const response = await enterpriseApiClient.post<{ result: FeeHead[] }>(
      `${this.BASE}/bulk`,
      payload
  ...
  static async searchFeeHeads(
    params: GetFeeHeadsRequest & { search?: string }
  ): Promise<GetFeeHeadsResponse["result"]> {
    const normalizedParams = {
      pageIndex: params?.pageIndex ?? API_CONFIG.PAGINATION.DEFAULT_PAGE_INDEX,
      pageSize: params?.pageSize ?? API_CONFIG.PAGINATION.DEFAULT_PAGE_SIZE,
