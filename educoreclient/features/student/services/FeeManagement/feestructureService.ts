...
export class FeeStructureService {
  ...
  static async getFeeStructures(params?: GetFeeStructuresRequest): Promise<GetFeeStructuresResponse['result']> {
    ...
    const response = await enterpriseApiClient.post<GetFeeStructuresResponse>(`${this.BASE}/organization`, normalizedParams);
    return response.result;
  }

  static async getFeeStructureById(id: string): Promise<FeeStructure> {
  const response = await enterpriseApiClient.get<FeeStructure>(`${this.BASE}/${id}`);
  return response;
  }

  static async createFeeStructure(request: CreateFeeStructureRequest): Promise<FeeStructure> {
    const response = await enterpriseApiClient.post<{ result: FeeStructure }>(this.BASE, request);
    return response.result;
  }

  static async updateFeeStructure(id: string, data: Partial<FeeStructureDto>): Promise<FeeStructure> {
    ...
    const response = await enterpriseApiClient.put<{ result: FeeStructure }>(this.BASE, payload);
    return response.result;
  }
  ...
  static async toggleFeeStructureStatus(id: string, completeData: FeeStructureDto, isActive: boolean): Promise<FeeStructure> {
    const payload = this.wrapFeeStructure({ ...completeData, id, isActive });
    const response = await enterpriseApiClient.put<{ result: FeeStructure }>(this.BASE, payload);
    return response.result;
  }

  static async bulkCreateFeeStructures(feeStructuresData: FeeStructureDto[]): Promise<FeeStructure[]> {
    const payload = { feeStructures: feeStructuresData.map(data => this.wrapFeeStructure(data)) };
    const response = await enterpriseApiClient.post<{ result: FeeStructure[] }>(`${this.BASE}/bulk`, payload);
    return response.result;
  }
  ...
  static async searchFeeStructures(params: GetFeeStructuresRequest & { search?: string }): Promise<GetFeeStructuresResponse['result']> {
    const normalizedParams = {
      pageIndex: params?.pageIndex ?? API_CONFIG.PAGINATION.DEFAULT_PAGE_INDEX,
      pageSize: params?.pageSize ?? API_CONFIG.PAGINATION.DEFAULT_PAGE_SIZE,
      organizationId: params?.organizationId?.toLowerCase(),
  ...
  static async exportFeeStructures(format: 'csv' | 'excel' = 'csv'): Promise<Blob> {
  ...
  static async getFeeHeadsByClass(request: FeeHeadsByClassRequest): Promise<FeeHeadByClassDto[]> {
    const response = await enterpriseApiClient.post<FeeHeadByClassDto[]>(`${this.BASE}/feeheads-by-class`, request);
    return response;
  }
}
