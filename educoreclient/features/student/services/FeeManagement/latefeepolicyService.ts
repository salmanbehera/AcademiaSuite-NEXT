...
export class LateFeePolicyService {
  ...
  static async getLateFeePolicies(params?: GetLateFeePoliciesRequest): Promise<GetLateFeePoliciesResponse['result']> {
    ...
    const response = await enterpriseApiClient.post<GetLateFeePoliciesResponse>(`${this.BASE}/organization`, normalizedParams);
  ...
  static async getLateFeePolicyById(id: string): Promise<LateFeePolicy> {
    const response = await enterpriseApiClient.get<{ result: LateFeePolicy }>(`${this.BASE}/${id}`);
    return response.result;
  }

  static async createLateFeePolicy(request: CreateLateFeePolicyRequest): Promise<LateFeePolicy> {
    const response = await enterpriseApiClient.post<{ result: LateFeePolicy }>(this.BASE, request);
    return response.result;
  }

  static async updateLateFeePolicy(id: string, data: Partial<LateFeePolicyDto>): Promise<LateFeePolicy> {
    ...
    const payload = this.wrapLateFeePolicy(merged);
    const response = await enterpriseApiClient.put<{ result: LateFeePolicy }>(this.BASE, payload);
    return response.result;
  }
  ...
  static async toggleLateFeePolicyStatus(id: string, completeData: LateFeePolicyDto, isActive: boolean): Promise<LateFeePolicy> {
    const payload = this.wrapLateFeePolicy({ ...completeData, id, isActive });
    const response = await enterpriseApiClient.put<{ result: LateFeePolicy }>(this.BASE, payload);
    return response.result;
  }

  static async bulkCreateLateFeePolicies(lateFeePoliciesData: LateFeePolicyDto[]): Promise<LateFeePolicy[]> {
    const payload = { lateFeePolicies: lateFeePoliciesData.map(data => this.wrapLateFeePolicy(data)) };
    const response = await enterpriseApiClient.post<{ result: LateFeePolicy[] }>(`${this.BASE}/bulk`, payload);
    return response.result;
  }
  ...
  static async searchLateFeePolicies(params: GetLateFeePoliciesRequest & { search?: string }): Promise<GetLateFeePoliciesResponse['result']> {
    const normalizedParams = {
      pageIndex: params?.pageIndex ?? API_CONFIG.PAGINATION.DEFAULT_PAGE_INDEX,
      pageSize: params?.pageSize ?? API_CONFIG.PAGINATION.DEFAULT_PAGE_SIZE,
      organizationId: params?.organizationId?.toLowerCase(),
  ...
  static async exportLateFeePolicies(format: 'csv' | 'excel' = 'csv'): Promise<Blob> {
